...
app.get("/books", async (req, res,next) => {
  try {
    ...
    res.render("home.ejs", { books, genres: allGenres, selectedGenres: [] })
  } catch (err) {
    next(err)
  }
})
...
app.get("/books/genre/filter", async (req, res,next) => {
  try {
    ...
    res.render("genre_filtering.ejs", { books, genre: genreDisplay })
  } catch (err) {
    next(err)
    next(err)
  }
...
app.get('/books/author/filter', async (req, res,next) => {
    try {
        ...
        res.render('author_filtering.ejs', { books, author });
    } catch (err) {
        next(err)
    }
});
...
app.get('/books/name/filter', async (req, res,next) => {
    try {
        ...
        res.render('name_filtering.ejs', { books, name });
    } catch (err) {
        next(err)
...
app.use((err, req, res, next) => {
  const { status = 500, message = "Error occurred" } = err
